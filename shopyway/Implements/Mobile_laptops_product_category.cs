...
{
    public class Mobile_laptops_product_category : IMobile_laptop_category_product
    {
        private readonly DataContext db;
        public Mobile_laptops_product_category(DataContext db)
        {
            this.db = db;
        }

        public dynamic get_mobile_laptop_category()
        {
            List<dynamic> Master_list = new List<dynamic>();
            List<Mobile_laptop_category> category = new List<Mobile_laptop_category>();
            ...
            var data1 = db.Mobile_accessory_category.FromSqlRaw("SELECT * FROM Mobile_accessory_category").ToList();
            ...
            var data = db.Mobile_accessory_product.FromSqlRaw("select top 14 * from Mobile_accessory_product ORDER BY NEWID()").ToList();
            for (int i = 0; i < 10; i++)
            {
